<template>
    <div class='page'>
        <van-tabs v-model="tabIndex">
            <van-tab title="未使用">
                <p v-if="this.useCouponsList.length===0" style='font-size:14px;color:#999;'>暂无卡券</p>
                <div class="listPage" v-else>
                    <div class="item" v-for="(item,index) in useCouponsList" :key='index'>
                        ...
                        <div :class="'itemUse ' + (item.coupon.type === 1?'':'i1')">
                            <van-button round type="danger" size='mini' @click="showManage(index)">使用</van-button>
                        </div>
                    </div>
                </div>
            </van-tab>
            <van-tab title="已使用">
                <p v-if="this.usedCouponsList.length===0" style='font-size:14px;color:#999;'>暂无卡券</p>
                <div class="listPage" v-else>
                    <div class="item" v-for="(item,index) in usedCouponsList" :key='index'>
                        <div :class="'itemContent ' + (item.coupon.type === 1?'':'i1')">
                            <p class="itemContent-title">{{item.coupon.name}}</p>
                            <p class="itemContent-time">已在 <span class="time">{{item.use_time}}</span> 使用</p>
                        </div>
                        <div :class="'itemUse ' + (item.coupon.type === 1?'':'i1')">
                            <van-button round type="danger" size='mini' @click="showManage(index)" disabled>使用</van-button>
                        </div>
                    </div>
                </div>
            </van-tab>
        ...
        <van-popup v-model="QrcodeManage" class="subscribeModal" v-if="QrcodeManage">
            ...
            <p class="qrcodeText" style="font-size:14px;">请给管理员展示此二维码进行核销</p>
        </van-popup>
    </div>
</template>
        ...
        qrcode(index) {
            this.$nextTick (() => {
                let qrcode = new QRCode("qrcode", {
                    width: 232, // 设置宽度
                    height: 232, // 设置高度
                    text: "https://zhlsqj.com/#/destroyCoupon?id="+ this.currentId
                });
            })
        },
        showManage(index) {
