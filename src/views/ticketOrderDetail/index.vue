<template>
	<div class="detail-page" v-if="show">
		...
		<div class="detail-page-conter-text">
			<p class="detail-page-conter-text-name">
				{{ orderData.fan_ticket[0].ticket.name }}
			</p>
			<p class="detail-page-conter-text-remark">
				{{ orderData.fan_ticket[0].ticket.remark }}
			</p>
			<p class="detail-page-conter-text-price">
				<span class="detail-page-conter-text-price-piece"
					>×{{ orderData.order_goods[0].num }}</span
				>
				<span class="detail-page-conter-text-price-price"
					>¥{{ orderData.order_goods[0].price }}</span
				>
			</p>
		</div>
		<!-- </div> -->
		<div class="detail-page-click">
			<p class="detail-page-click-p">
				...
				<span class="detail-page-click-p-allPrice"
					>¥
					{{
						orderData.order_goods[0].price * orderData.order_goods[0].num
					}}</span
				>
			</p>
			<van-button
			...
			<van-button
				v-if="
					orderData.pay_state === 1 && orderData.use_state === 0 && hasRefun
				"
				class="detail-page-click-intoDefail"
				round
				type="warning"
		...
		<div class="detail-page-detail">
			...
			<p class="detail-page-detail-p">
				...
				<span class="detail-page-detail-p-price">{{
					orderData.fan_ticket[0].name
				}}</span>
			</p>
			<p class="detail-page-detail-p">
				<span class="detail-page-detail-p-text">电话</span>
				<span class="detail-page-detail-p-price">{{
					orderData.fan_ticket[0].mobile
				}}</span>
			</p>
			<p class="detail-page-detail-p">
				<span class="detail-page-detail-p-text">预定日期</span>
				<span class="detail-page-detail-p-price">{{
					orderData.fan_ticket[0].booking_date
				}}</span>
			</p>
		</div>
		<van-popup
		...
		>
			...
			<p class="qrcodeText" style="font-size:14px;">
				请给店员展示此二维码进行使用
			</p>
		</van-popup>
	</div>
		...
		qrcode() {
			this.$nextTick(() => {
				let qrcode = new QRCode("qrcode", {
					...
					height: 232, // 设置高度
					text:
						"https://lsqj.rdoorweb.com/#/destroyTicket?id=" + this.orderData.id
				});
			});
		},
		showQrcode() {
		...
		getOrderData() {
			axios
				...
				.then(res => {
					...
					this.show = true;
					if (
						new Date(this.orderData.fan_ticket[0].booking_date).getTime() >
							new Date().getTime() &&
						this.use_state > -1
					) {
						this.hasRefun = true;
					...
					if (
						new Date(this.orderData.fan_ticket[0].booking_date).getDate() ===
						new Date().getDate()
					) {
