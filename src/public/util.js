...
class Token {
    ...
    getToken() {
            ...
            if (router.history.current.query.id) {
                this.url = encodeURIComponent(
                    "http://localhost:8081/#" +
                        ...
                        "?id=" +
                        router.history.current.query.id
                )
            } else {
                this.url = encodeURIComponent(
                    "http://localhost:8081/#" + router.history.current.path
                )
            }
        } else {
            //上传服务器使用
        ...
        location.href = baseURL + "wechat/oauth?url=" + this.url
    }
